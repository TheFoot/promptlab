...
async function generateFromQuestionnaire(answers) {
  try {
    // Build a prompt template to send to the AI analysis API
    ...
    console.log('Falling back to mock implementation - unexpected API response format');
...
`;
    ...
    if (result && result.suggestions && result.suggestions.length > 0) {
      ...
      let bestSuggestion = result.suggestions.reduce((best, current) => {
        if (!best.replacementText) return current;
        if (!current.replacementText) return best;
        return current.replacementText.length > best.replacementText.length ? current : best;
      }, { replacementText: '' });
      
      // Return the refined content
      if (bestSuggestion.replacementText && bestSuggestion.replacementText.length > content.length / 2) {
        return {
          content: bestSuggestion.replacementText,
          suggestedChanges: [
            {
              type: bestSuggestion.category || 'refinement',
              description: bestSuggestion.description || 'Improved based on your instructions',
...
`;
    ...
    return { 
      sections: sections.reduce((acc, section) => {
        acc[section.id] = generateMockSectionContent(type, section.name || section.id);
        return acc;
      }, {})
    };
  } catch (error) {
 ...
 * @param {string} purpose - The prompt purpose
 * @returns {Promise<Object>} Analysis results and suggestions
 */
async function analyzePrompt(content, purpose) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/ai-analysis/analyze`, {
      ...
      body: JSON.stringify({ 
        promptText: content,
        analysisAspects: ['clarity', 'specificity', 'structure', 'completeness'],
...
`;
    ...
    if (!response.ok) {
      throw new Error(`Failed to generate improvements: ${response.statusText}`);
    }
...
`;
    ...
    if (tags.length === 0 && result.suggestions && result.suggestions.length > 0) {
      // Try to find a suggestion that mentions tags
      const tagSuggestion = result.suggestions.find(s => 
        s.description && s.description.toLowerCase().includes('tag')
      );
      
      if (tagSuggestion && tagSuggestion.replacementText) {
        const extractedTags = tagSuggestion.replacementText.split(/[,\s]+/).filter(tag => 
...
function extractTagsFromContent(content) {
  ...
  const typeIndicators = ['code', 'creative', 'write', 'generate', 'analyze', 'explain', 'summarize', 'compare'];
  ...
  for (const line of lines) {
    if (line.startsWith('## ')) {
      ...
      if (['instructions', 'context', 'examples', 'format', 'output'].includes(sectionName)) {
...
`;
    ...
    return {
      ...
      differences: result.suggestions ? result.suggestions.map(s => ({
...
function getComponentDescriptions(components) {
  if (!components || !Array.isArray(components) || components.length === 0) {
    return ['Relevant information on the subject', 'Clear explanation of key concepts'];
  }
  ...
  return descriptions.length > 0 ? descriptions : ['Clear explanation of key concepts'];
}
