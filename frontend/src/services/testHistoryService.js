...
async function saveTestSession(promptId, conversation, metrics = {}) {
  try {
    const response = await fetch(
      `${API_BASE_URL}/api/prompts/${promptId}/tests`,
      {
        method: "POST",
        headers: {
...
async function getTestSession(promptId, sessionId) {
  try {
    const response = await fetch(
      `${API_BASE_URL}/api/prompts/${promptId}/tests/${sessionId}`,
    );
  ...
  try {
    ...
    if (options.startDate) {
      ...
      sessions = sessions.filter(
        (session) => new Date(session.timestamp).getTime() >= startDate,
      );
    }
    ...
    if (options.endDate) {
      ...
      sessions = sessions.filter(
        (session) => new Date(session.timestamp).getTime() <= endDate,
      );
    }
...
async function getTestHistoryWithFallback(promptId, options = {}) {
  try {
    ...
    console.warn(
      "Failed to get test history from API, falling back to localStorage:",
      error,
    );
