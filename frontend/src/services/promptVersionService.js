...
async function getVersion(promptId, versionId) {
  try {
    const response = await fetch(`${API_BASE_URL}/api/prompts/${promptId}/versions/${versionId}`);
...
async function restoreVersion(promptId, versionId) {
  try {
    const response = await fetch(`${API_BASE_URL}/api/prompts/${promptId}/versions/${versionId}/restore`, {
      method: 'PUT',
    });
    
    if (!response.ok) {
      throw new Error(`Failed to restore prompt version: ${response.statusText}`);
    }
...
async function getVersionsWithFallback(promptId) {
  try {
    ...
    console.warn('Failed to get versions from API, falling back to localStorage:', error);
...
async function saveVersionWithFallback(promptId, content, metadata = {}) {
  try {
    ...
    console.warn('Failed to save version to API, falling back to localStorage:', error);
