...
async function analyzePrompt(promptText, options = {}) {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.message || `Failed to analyze prompt: ${response.statusText}`);
    }
...
async function submitAnalysisFeedback(analysisId, feedbackType, comments = '') {
  try {
    const response = await fetch(`${API_BASE_URL}/api/prompts/analysis-feedback`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
async function getAnalysisTemplates() {
  try {
    const response = await fetch(`${API_BASE_URL}/api/prompts/analysis-templates`);

    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(errorData.message || `Failed to fetch templates: ${response.statusText}`);
    }
...
async function getPromptSuggestionHistory(promptId) {
  try {
    const response = await fetch(`${API_BASE_URL}/api/prompts/${promptId}/suggestion-history`);

    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(errorData.message || `Failed to fetch suggestion history: ${response.statusText}`);
    }
