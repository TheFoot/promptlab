  ...
  switch (currentStep.value) {
    ...
    case 3:
      if (creationMethod.value === "template") {
        ...
        return (
          !isNil(aiGeneratedContent.value) && aiGeneratedContent.value !== ""
        );
      }
...
function goToStep(stepIndex) {
  ...
  if (
    ...
    (stepIndex === currentStep.value + 1 && canProceed.value)
  ) {
  ...
  switch (stepIndex) {
    case 0: // Purpose
      if (selectedPurpose.value) {
        const purpose = purposes.value.find(
          (p) => p.id === selectedPurpose.value,
        );
...
function preparePromptContent() {
  console.log(
    "preparePromptContent called, creationMethod:",
  ...
  if (creationMethod.value === "template" && selectedTemplate.value) {
    ...
    console.log(
      ...
      aiGeneratedContent.value.substring(0, 50) + "...",
    );
  ...
  console.log(
    "promptContent is now set to length:",
    promptContent.value.length,
  );
}
...
function getBlankTemplate() {
  ...
  const purposeText =
    purposes.value.find((p) => p.id === selectedPurpose.value)?.name || "";
  const typeText =
    promptTypes.value.find((t) => t.id === selectedType.value)?.name || "";
...
onMounted(async () => {
  try {
    ...
    await Promise.all([loadTemplates(), loadPromptTypes()]);
  } catch (error) {
  ...
  try {
    ...
    promptTypes.value = [
      ...
      },
      {
        ...
        example:
          "Analyze this sales data and identify key trends and outliers.",
      },
    ];
  }
}
...
.enhanced-prompt-creator {
  ...
  .creator-header {
    ...
    background-color: var(
      --step-header-bg,
      var(--step-number-bg, #f0f0f0)
    ); /* Use a slightly different variable for dark mode */

    @media (prefers-color-scheme: dark) {
      background-color: color-mix(
        in srgb,
        var(--step-number-bg, #333) 95%,
        black
      ); /* Very subtle darker shade in dark mode */
    }
      ...
      .progress-step {
        ...
        &.completed {
          ...
          .step-name {
            color: var(
              --primary-color
            ); /* Ensure active step name is highlighted */
          }
        }
  ...
  .creator-content {
    ...
    max-height: calc(
      100vh - 220px
    ); /* Further adjusted for increased header spacing */
    background-color: var(
      --bg-color,
      white
