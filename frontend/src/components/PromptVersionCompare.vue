      ...
      <div class="versions-metadata">
        <div class="version-info">
          ...
          <div class="metadata-item">
            ...
            <span class="metadata-value">{{ formatDateLong(comparisonData.version1.createdAt) }}</span>
          </div>
          <div
          ...
          >
            ...
            <span class="metadata-value">{{ comparisonData.version1.metadata.description }}</span>
          </div>
          <div
          ...
          >
            ...
            <span class="metadata-value">{{ comparisonData.version1.metadata.changedBy }}</span>
          </div>
        ...
        <div class="version-info">
          ...
          <div class="metadata-item">
            <span class="metadata-label">Created:</span>
            <span class="metadata-value">{{ formatDateLong(comparisonData.version2.createdAt) }}</span>
          </div>
          <div
          ...
          >
            <span class="metadata-label">Description:</span>
            <span class="metadata-value">{{ comparisonData.version2.metadata.description }}</span>
          </div>
          <div
          ...
          >
            <span class="metadata-label">Changed by:</span>
            <span class="metadata-value">{{ comparisonData.version2.metadata.changedBy }}</span>
          </div>
          <div class="version-actions">
            <button 
              class="btn-restore" 
        ...
        <div class="metrics-grid">
          ...
          <div class="metric-column">
            ...
            <div
              ...
              class="metric-row"
            >
              <span>{{ formatMetricValue(comparisonData.version1.testResults?.[key]) }}</span>
            </div>
          </div>
          ...
          <div class="metric-column">
            ...
            <div
              ...
              class="metric-row"
            >
              <span>{{ formatMetricValue(comparisonData.version2.testResults?.[key]) }}</span>
            </div>
          </div>
...
const hasTestResults = computed(() => {
  return comparisonData.value && 
    (comparisonData.value.version1.testResults || comparisonData.value.version2.testResults);
});
...
const diffLines = computed(() => {
  ...
  const unchangedMap = new Map(unchanged.map(item => [item.line, item.content]));
...
watch(selectedVersionIds, async (newIds, oldIds) => {
  if ((newIds.version1 && newIds.version2) && 
      (newIds.version1 !== oldIds.version1 || newIds.version2 !== oldIds.version2)) {
    await loadVersions();
  }
}, { deep: true });
...
async function loadVersionsList() {
  ...
  try {
    ...
    const versionsList = await promptVersionService.getVersionsWithFallback(props.promptId);
...
async function loadVersions() {
  if (!selectedVersionIds.value.version1 || !selectedVersionIds.value.version2) {
...
async function restoreVersion(versionId) {
  ...
  const confirmation = confirm('Are you sure you want to restore this version? This will replace the current prompt content.');
