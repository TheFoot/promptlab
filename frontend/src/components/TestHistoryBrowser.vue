        ...
        >
          ...
          <div class="session-metrics">
            <div class="metric">
              ...
              <span class="metric-value">{{ formatMetric(session.metrics?.responseTime, 'ms') }}</span>
            </div>
            <div class="metric">
              <span class="metric-label">Tokens:</span>
              <span class="metric-value">{{ formatMetric(session.metrics?.tokenCount) }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    >
      ...
      <div class="session-info">
        <div class="info-item">
          ...
          <span class="info-value">{{ formatDateLong(selectedSession.timestamp) }}</span>
        </div>
        <div class="info-item">
          <span class="info-label">Model:</span>
          <span class="info-value">{{ getModelName(selectedSession.modelId) }}</span>
        </div>
        <div class="info-item">
          <span class="info-label">Temperature:</span>
          <span class="info-value">{{ selectedSession.parameters?.temperature || 'N/A' }}</span>
        </div>
        <div class="info-item">
          <span class="info-label">Top P:</span>
          <span class="info-value">{{ selectedSession.parameters?.top_p || 'N/A' }}</span>
        </div>
      </div>
...
async function loadHistory() {
  ...
  try {
    ...
    const result = await testHistoryService.getTestHistoryWithFallback(props.promptId, options);
...
async function loadSessionDetail(sessionId) {
  try {
    ...
    const session = await testHistoryService.getTestSession(props.promptId, sessionId);
    selectedSession.value = session;
  } catch (error) {
    console.error('Error loading session detail:', error);
    // Try to use the session from the list as a fallback
    selectedSession.value = testSessions.value.find(s => s.id === sessionId) || null;
  }
}

function selectSession(sessionId) {
  selectedSessionId.value = sessionId === selectedSessionId.value ? null : sessionId;
}
