...
describe("modelConfigService", () => {
  ...
  it("provider helper methods work correctly", async () => {
    ...
    const displayName =
      await modelConfigService.getProviderDisplayName("openai");
    expect(displayName).toBe("OpenAI");

    // Test with unknown provider
    const unknownDisplayName =
      await modelConfigService.getProviderDisplayName("unknown");
  ...
  it("model helper methods work correctly", async () => {
    ...
    const availableModels =
      await modelConfigService.getAvailableModels("openai");
    ...
    const displayName = await modelConfigService.getModelDisplayName(
      "openai",
      "gpt-3.5-turbo",
    );
    ...
    // Test with unknown provider
    const unknownProviderModels =
      await modelConfigService.getAvailableModels("unknown");
    expect(unknownProviderModels).toEqual([]);

    const unknownProviderDefault =
      await modelConfigService.getDefaultModel("unknown");
    expect(unknownProviderDefault).toBe("");

    // Test with unknown model
    const unknownModelDisplay = await modelConfigService.getModelDisplayName(
      "openai",
      "unknown-model",
    );
