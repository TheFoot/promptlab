...
describe('promptStore', () => {
  ...
  it('fetches a prompt by ID successfully', async () => {
    const mockPrompt = { _id: '1', title: 'Prompt 1', content: 'Content 1', tags: ['tag1'] };
  ...
  it('creates a prompt successfully', async () => {
    const newPrompt = { title: 'New Prompt', content: 'New Content', tags: ['new'] };
  ...
  it('updates a prompt successfully', async () => {
    ...
    const updatedPrompt = { _id: promptId, title: 'Updated Title', content: 'Updated Content', tags: ['tag1'] };
    ...
    store.currentPrompt = { _id: '1', title: 'Prompt 1', content: 'Content 1', tags: ['tag1'] };
    
    // Call updatePrompt
    const result = await store.updatePrompt(promptId, updateData);
    
    // Check that the axios.put was called correctly
    expect(axios.put).toHaveBeenCalledWith(`/api/prompts/${promptId}`, updateData);
  ...
  it('sets search query and fetches prompts', async () => {
    ...
    expect(axios.get).toHaveBeenCalledWith('/api/prompts', { params: { search: 'test query' } });
  });
  ...
  it('sets selected tag and fetches prompts', async () => {
    ...
    // Check that fetchPrompts was called with the correct params
    expect(axios.get).toHaveBeenCalledWith('/api/prompts', { params: { tag: 'javascript' } });
  });
