...
export async function analyzePrompt(req, res) {
  try {
    const { promptText, model, analysisAspects, includeAlternatives = true } = req.body;
    ...
    const response = await providerClient.generateAnalysis(messages, { includeAlternatives });
    
    // Process and return the response
    return res.status(200).json(response);
  } catch (error) {
    global.logger.error('Error analyzing prompt', error);
    return res.status(500).json({ message: 'Error analyzing prompt', error: error.message });
  }
}
...
export async function submitAnalysisFeedback(req, res) {
  try {
    ...
    console.log(`Feedback received for analysis ${analysisId}:`, { feedbackType, comments });
...
export async function getPromptSuggestionHistory(req, res) {
  try {
    ...
    return res.status(500).json({ message: 'Error fetching suggestion history', error: error.message });
  }
}
...
export async function saveTestSession(req, res) {
  try {
    ...
    return res.status(500).json({ message: 'Error saving test session', error: error.message });
  }
}
...
export async function getTestHistory(req, res) {
  try {
    ...
    let filteredSessions = testSessions.filter(session => session.promptId === promptId);

    // Apply additional filters
    if (startDate) {
      const start = new Date(startDate).getTime();
      filteredSessions = filteredSessions.filter(session => new Date(session.timestamp).getTime() >= start);
    }

    if (endDate) {
      const end = new Date(endDate).getTime();
      filteredSessions = filteredSessions.filter(session => new Date(session.timestamp).getTime() <= end);
    }

    if (modelId) {
      filteredSessions = filteredSessions.filter(session => session.modelId === modelId);
    }
    ...
    return res.status(500).json({ message: 'Error fetching test history', error: error.message });
  }
}
...
export async function getTestSession(req, res) {
  try {
    ...
    if (!promptId || !sessionId) {
      return res.status(400).json({ message: 'Prompt ID and session ID are required' });
    }

    // Find the session
    const session = testSessions.find(s => s.id === sessionId && s.promptId === promptId);
    ...
    return res.status(500).json({ message: 'Error fetching test session', error: error.message });
  }
}
...
export async function deleteTestSession(req, res) {
  try {
    ...
    const sessionIndex = testSessions.findIndex(s => s.id === sessionId && s.promptId === promptId);
    ...
    return res.status(200).json({ message: 'Test session deleted successfully' });
  } catch (error) {
    global.logger.error('Error deleting test session', error);
    return res.status(500).json({ message: 'Error deleting test session', error: error.message });
  }
}
...
export async function savePromptVersion(req, res) {
  try {
    ...
    const promptVersionsList = promptVersions.filter(v => v.promptId === promptId);
    
    // Calculate next version number
    const versionNumber = promptVersionsList.length > 0
    ...
    return res.status(500).json({ message: 'Error saving prompt version', error: error.message });
  }
}
...
export async function getPromptVersions(req, res) {
  try {
    ...
    return res.status(500).json({ message: 'Error fetching prompt versions', error: error.message });
  }
}
...
export async function getPromptVersion(req, res) {
  try {
    ...
    // Find the version
    const version = promptVersions.find(v => v.id === versionId && v.promptId === promptId);
    ...
    return res.status(500).json({ message: 'Error fetching prompt version', error: error.message });
  }
}
...
export async function generatePrompt(req, res) {
  try {
    ...
    const systemPrompt = `You are an expert AI prompt engineer. Your task is to create a well-structured, effective prompt based on the questionnaire answers provided.
Create a prompt that matches the requirements in the questionnaire answers exactly.
Format your response as a complete, well-structured markdown document with appropriate headings and sections.
Focus on clarity, specificity, and providing enough context for the AI to understand the request precisely.
The prompt should be ready to use without further modification.`;

    // Build messages array for the AI
    const messages = [
      { role: 'system', content: systemPrompt },
      { role: 'user', content: `Please create a prompt based on these questionnaire answers:\n\n${questionnaire}` }
    ];
    ...
    console.log('Returning response with content length:', promptContent.length, 'title:', title);
    ...
    return res.status(500).json({ message: 'Error generating prompt', error: error.message });
  }
}
...
export async function restorePromptVersion(req, res) {
  try {
    ...
    return res.status(500).json({ message: 'Error restoring prompt version', error: error.message });
  }
}
 ...
 */
function buildAnalysisSystemPrompt(aspects = ['clarity', 'conciseness', 'context', 'specificity', 'formatting']) {
  return `You are an expert AI prompt engineer. Analyze the given prompt and provide constructive feedback to improve it.
...
async function getProviderClient(provider, modelName) {
  ...
  return {
    generateAnalysis: async (messages, options) => {
      ...
      return {
        overallScore: 75,
        summary: "Your prompt is generally clear but could be more specific in certain areas. Adding more context would help the AI understand exactly what you're looking for.",
        suggestions: [
          {
            category: "clarity",
            title: "Clarify the main objective",
            description: "The prompt's main goal is somewhat ambiguous. Making it explicit will help the AI focus on what's most important.",
            originalText: "I want you to analyze this data and give me insights.",
            replacementText: "I want you to analyze this financial data and give me insights about revenue trends over the past quarter.",
            alternatives: [
              { text: "Analyze this financial data and identify the top 3 factors affecting our revenue this quarter." },
              { text: "Review this financial dataset and provide specific insights on cost reduction opportunities and revenue growth." }
            ]
          },
          {
            category: "specificity",
            title: "Add specific examples",
            description: "Including examples of the type of insights you're looking for would help guide the response.",
            originalText: "",
            replacementText: "For example, I'd like to know if there are any unexpected spikes or drops in our monthly sales figures, and what might have caused them.",
            alternatives: []
          }
        ]
      };
    ...
    generateContent: async (messages) => {
      ...
      // Extract the purpose and type from messages to customize the response
      const userMessage = messages.find(m => m.role === 'user')?.content || '';
      const promptType = (userMessage.match(/promptType:\s*(\w+)/) || [])[1] || 'general';
      const promptPurpose = (userMessage.match(/promptPurpose:\s*(\w+)/) || [])[1] || 'information';
...
- If you have questions about the requirements, please ask for clarification before proceeding.`;
      } else if (promptType === 'creative' || userMessage.includes('creative')) {
...
function getReadablePurpose(purposeId) {
  switch(purposeId) {
    case 'information': return 'Information Extraction';
    case 'generation': return 'Content Generation';
    case 'transformation': return 'Content Transformation';
    case 'analysis': return 'Analysis & Reasoning';
    case 'conversation': return 'Conversation Design';
    default: return purposeId ? purposeId.charAt(0).toUpperCase() + purposeId.slice(1) : 'General';
  }
}
...
function getReadableType(typeId) {
  switch(typeId) {
    ...
    default: return typeId ? typeId.charAt(0).toUpperCase() + typeId.slice(1) : 'Custom';
  }
}